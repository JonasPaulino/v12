...
class DesktopSyncDAO {
  ...
  static async listarProdutos(client, { tenantId, since = null, limit = 1000 }) {
    ...
    const { rows } = await client.query(
      `
        SELECT
          ...
          COALESCE(pf.ibpt_aliquota_federal_nacional, 0) AS ibpt_aliquota_federal_nacional,
          COALESCE(pf.ibpt_aliquota_federal_importado, 0) AS ibpt_aliquota_federal_importado,
          COALESCE(pf.ibpt_aliquota_estadual, 0) AS ibpt_aliquota_estadual,
          COALESCE(pf.ibpt_aliquota_municipal, 0) AS ibpt_aliquota_municipal,
          pf.ibpt_fonte,
          pf.ibpt_chave,
          pf.ibpt_atualizado_em,
    ...
    return rows.map((row) => ({
      ...
      ibpt_aliquota_federal_nacional: Number(row.ibpt_aliquota_federal_nacional || 0),
      ibpt_aliquota_federal_importado: Number(row.ibpt_aliquota_federal_importado || 0),
      ibpt_aliquota_estadual: Number(row.ibpt_aliquota_estadual || 0),
      ibpt_aliquota_municipal: Number(row.ibpt_aliquota_municipal || 0),
